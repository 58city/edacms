 ...
 * Main Controller
 */
angular.module('controllers').controller('main', ['$rootScope','$scope', '$http','$window',function ($rootScope,$scope, $http,$window) {
	'use strict';
	/**
	 * 初始化变量
	 */
	 ...
	 * NoderCMS 官方信息
	 */
	// $http.get('http://console.nodercms.com/openApi/info').then(function (res) {
	// 	var data = res.data;
...
angular.module('services').factory('base64ToBlobFile', [function () {
    ...
    return function (base64Data, fileName, contentType) {
		...
		// 获取字符数量（每个ascii字符占一个字节）  
		var bytesLength = byteCharacters.length;
		// 所有字符划分为若干个区域，每个区域包含1024个字符 
		var slicesCount = Math.ceil(bytesLength / sliceSize);
		// 创建一个数组，成员数量为划分的区域数量，每个成员为包含1024个成员的Uint8Array的类型数组
      	...
      	for (var sliceIndex = 0; sliceIndex < slicesCount; ++sliceIndex) {
			...
			// 把当前区域所在普通数组转换为Uint8Array类型数组，并添加到区域数组中
 ...
 * 判断是否已安装
 */
angular.module('services').factory('checkInstallResolve', ['$q', '$state', '$http',function ($q, $state, $http) {
    'use strict';
    return {
      	leaveToSignInOrNone: function () {
        	var deferred = $q.defer();
        	$http.get('/api/install').then(function (res) {
            	var data = res.data;
            	if (!data.hasInstall) {
              		deferred.resolve();
            	} else {
 ...
 * 汉字转拼音
 */
angular.module('services').factory('pinyin', ['$http', '$q',function ($http, $q) {
	'use strict';
	/**
	 *   UTF-16汉字编码与拼音对照表
	 */
